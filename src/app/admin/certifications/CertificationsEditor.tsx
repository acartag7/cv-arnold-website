...
/**
 * Certifications Editor Container
 *
 * Manages CRUD operations for professional certifications.
 * Similar pattern to Experience editor with sortable list.
 *
...
export function CertificationsEditor() {
  ...
  const handleDeleteClick = useCallback((certification: Certification) => {
    setDeleteConfirm({ isOpen: true, certification })
  }, [])
  ...
  const handleSave = useCallback(
    (certification: Certification) => {
      ...
      const existingIndex = data.certifications.findIndex(
        c => c.id === certification.id
      )
      let updatedCertifications: Certification[]

      if (existingIndex >= 0) {
        updatedCertifications = [...data.certifications]
        updatedCertifications[existingIndex] = certification
      } else {
        const newCert = { ...certification, order: data.certifications.length }
  ...
  const handleDeleteConfirm = useCallback(() => {
    if (!data || !deleteConfirm.certification) return

    const updatedCertifications = data.certifications
      .filter(c => c.id !== deleteConfirm.certification!.id)
  ...
  }, [data, deleteConfirm.certification, updateData, toast, handleDeleteCancel])

  const handleReorder = useCallback(
    (reorderedCertifications: Certification[]) => {
      if (!data) return

      const updated = reorderedCertifications.map((cert, index) => ({
        ...cert,
        order: index,
      }))
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${deleteConfirm.certification?.name}"? This action cannot be undone.`}
        confirmText="Delete"
        variant="danger"
        isLoading={isSaving}
      />
